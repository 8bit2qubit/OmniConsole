...
- **系統管理員程式支援（專業版）** – 手把映射能作用於以系統管理員身分執行的程式與遊戲。在**設定 → 進階 → 貓又**點選**安裝**並通過系統管理員授權，此後你的設定檔、貓又模式與 Steam 遊戲內嵌介面快捷鍵，在這類程式中都與在其他程式中一樣運作，OmniCharm 小工具也能為它們開啟映射編輯器。
- **用手把叫出螢幕鍵盤（專業版）** – Windows 本身就能從 Game Bar 叫出螢幕鍵盤，那條路人人都能用。專業版多提供一條直達路線：把任一按鍵映射成叫出鍵盤，手邊的事不必中斷。可選用手把直接打字的**遊戲控制器鍵盤**，或以游標點按的**螢幕小鍵盤**。螢幕小鍵盤另需系統管理員程式支援。
...
- **OmniConsole 專業版** – 設定中的**專業版**分頁。OmniConsole 是獨立開發者利用自己的閒暇時間撰寫與維護的個人興趣專案，你的贊助能讓它繼續走下去。升級為專業版即可解鎖系統管理員程式支援與自訂控制器版面。該分頁備有贊助連結，升級也在此完成，並會顯示授權對象，以及這台主機持有的授權。
