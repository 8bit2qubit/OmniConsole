...
- **XBOX Mode (FSE) entry & exit** – When you launch OmniConsole outside XBOX Mode (FSE) (e.g., from the Start Menu), it automatically triggers the XBOX Mode (FSE) entry dialog. Under **Settings → Advanced → XBOX Mode (FSE)**, choose what happens when you enter from the desktop: **Ask every time**, **Restart (optimized performance)**, or **Start now (no performance changes)**. The confirmation on the way back to the Windows desktop is yours to keep or leave out as well.
...
- **OmniCharm widget** – A Game Bar widget for in-game quick access. Open **Task View**, the **XBOX Library**, **File Explorer**, or the **Steam Overlay** in one tap; toggle **Nekomata Mode**, controller layout preset, cursor speed and size, and the **Steam In-Game Overlay** (long-press ☰). File Explorer opens straight to **This PC**, and Nekomata Mode's built-in mapping already covers it, so you browse and open your files with the gamepad alone.
...
- **Cursor speed & size** – Under **Settings → Advanced → Nekomata** you can set the cursor’s movement speed from 25% to 200%, and its size from 100% up to 800%. The OmniCharm widget carries the same two sliders.
...
- **Nekomata Mode on the ROG Ally family (Pro)** – These consoles ship with the manufacturer's own gamepad mapping, and Nekomata defers to it. Pro makes that a choice: turn it on under **Settings → Advanced**, then set **Control Mode** to **Gamepad** in Command Center. Everything Nekomata does on a desktop she does here too.
