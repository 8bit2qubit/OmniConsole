...
namespace OmniConsole.Dialogs
{
    ...
    {
        /// <summary>使用者按下 primary 按鈕（確定/是）後為 true；否則為 false。</summary>
        public bool Result { get; private set; }

        /// <summary>
        /// 建立訊息/確認對話方塊；closeText 傳 null 表示「純資訊單按鈕」，primaryText 當關閉按鈕字樣。
        /// defaultToPrimary=true 時預設醒目 Primary 按鈕（如「立即重啟」方便直接確認）；預設 false＝醒目 Close
        ...
        /// <summary>
        /// defaultToPrimary 時，排進 dispatcher 佇列等樣板套用後，把焦點設到樣板的 Primary 按鈕。
